<?php

include "konekweb.php";

if (!$conn) {
    die("Connection failed:". mysqli_connect_error());
} echo "Database Connection is OK <br>";
       
    $servo = $_GET['servo'];
    $berhascacah = $_GET['berhascacah'];
    $tanggal = date("Y-m-d H:i:s", strtotime('+7 hours'));
    $KODE = $_GET['KODE'];
    $servo_id = $_GET['servo_id'];
    
    $upservo_stmt = $conn->prepare("UPDATE updata SET `servo` = ?,`berhascacah` = ?,`tanggal` = ?  WHERE KODE = '$KODE'");
    $upservo_stmt->bind_param("sss",$servo,$berhascacah,$tanggal);

    $users_stmt = $conn->prepare("UPDATE users SET `servo` = ? WHERE `servo` = '$servo_id'");
    $users_stmt->bind_param("s",$servo);

    $upservo_stmt->execute();
    $users_stmt->execute();

    // $id = 1; // Replace this with the actual id value you want to update
    // $stmt->execute();

    // Check if the update was successful
    if ($upservo_stmt->affected_rows > 0) {
        echo "Update tabel updata untuk servo successful <br>";
    } else {
        echo "Update servo di updata tabel failed";
    }

    if ($users_stmt->affected_rows > 0) {
        echo "Update servo users successful <br>";
    } else {
        echo "Update servo user failed";
    }

    // Close statement
    $upservo_stmt->close();
    $users_stmt->close();

    // Close connection
    $conn->close();
    
?>